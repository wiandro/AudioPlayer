package com.wiandro.audioplayer.playlist
...
class PlaylistManager(private val mListener: OnSongUpdateListener) {
    ...
    fun getCurrentSong(): ASong? =playlist?.getItem(mCurrentIndex)
    ...
    private fun setCurrentPlaylistIndex(index: Int) {
        if (index >= 0 && index < playlist?.getCurrentPlaylistSize() ?: 0) {
            mCurrentIndex = index
    ...
    fun isRepeat(): Boolean = playlist?.isRepeat ?: false
